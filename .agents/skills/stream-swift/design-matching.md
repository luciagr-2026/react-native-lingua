...
- **Any surface or chrome you hand-build binds to the SDK's adaptive semantic tokens, not literals — the same discipline as sizing above.** A custom header, row background, composer wrapper, side panel, or tab bar you build yourself is a *structural surface*: back it with the SDK's dynamic tokens — `Color(colors.backgroundCoreApp)` for the list-level background, `Color(colors.backgroundCoreElevation1)` for bars/fields (`@Injected(\.colors) var colors`, from `StreamChatCommonUI`) — **never** a pinned `Color.white` or sampled literal. The reference is almost always a *light* screenshot, so a pinned surface looks right in light mode but stays light under dark chrome (a hard light/dark split). **Brand and content colors stay pinned** — sampled bubble fills, glyphs, wallpaper, ticks are identical in light and dark, so pinning them is correct and keeps them from washing out. Pin content; keep the surface semantic. (For the exact SDK-slot → token mapping when you override the SDK's *own* surface modifiers rather than hand-building, see the color carve-out under "Follow EVERY color from the reference".) Verify on what you just built, not just the SDK regions: toggle `xcrun simctl ui <udid> appearance dark` and confirm your custom surfaces flip while the brand colors hold.
...
- **Carve-out — structural SURFACES stay adaptive; do NOT pin them to a sampled literal.** Sample-and-pin is right for **brand / content** colors — the sampled bubble fills, glyphs, wallpaper, and read-receipt ticks: those are identical in light and dark, so keep them pinned. But when you override the SDK's **own** surface modifiers, bind them to the SDK's **semantic surface tokens, which already adapt to light/dark** — dynamic `UIColor(light:dark:)` values in `Appearance.ColorPalette` (`StreamChatCommonUI`) — **not** a hardcoded `Color.white` or sampled literal. Use the same token the SDK default uses for each surface: the **message-list background** (`makeMessageListBackground`) → `backgroundCoreApp`; the **composer bar** (`Styles.makeComposerViewModifier`) and the **input-field container** (`Styles.makeComposerInputViewModifier`) → `backgroundCoreElevation1`, applied as `.background(Color(colors.backgroundCoreElevation1))` (with `@Injected(\.colors) var colors`). Pin the sampled bubble fills; keep the surface semantic. (For chrome you **hand-build** rather than override — a custom header, row, panel, tab bar — the same principle and the dark-mode verify step live in the hand-build binding rule in the custom-chrome cluster under the Liquid Glass section.)
